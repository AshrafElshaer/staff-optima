...
export function BlockSuggestionCard({
	...
	return (
		/** biome-ignore lint/a11y/noStaticElementInteractions: default suggestion card*/
		<div
							...
							<React.Fragment>
								{suggestionText2Array(suggestion.text!).map((text, index) => (
									...
									>
										...
										<span key={index.toString()} className="text-sm">
											{text}
										</span>
									</div>
								))}
	...
	const resolvedSuggestion: ResolvedSuggestion[] = React.useMemo(() => {
		...
		suggestionIds.forEach((id) => {
			...
			const nodeData = api.suggestion.suggestionData(
				entries[0]?.[0] as TElement | TSuggestionText,
			);
